...
export function SunArcCard({ sunrise, sunset, currentTime }: SunArcCardProps) {
  ...
  return (
    <section className="wx-card" aria-label={t('sun.title')}>
      ...
      <div className="wx-sun-wrap">
        ...
        <div className="wx-sun-times">
          <div>
            <span className="lbl">{t('sun.sunrise')}</span>
            <span className="val">{formatTime(sunrise, i18n.language)}</span>
          </div>
          <div className="r">
            <span className="lbl">{t('sun.sunset')}</span>
            <span className="val">{formatTime(sunset, i18n.language)}</span>
          </div>
        </div>
      </div>
    </section>
