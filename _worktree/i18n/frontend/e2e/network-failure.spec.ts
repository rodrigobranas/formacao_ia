...
test.describe('Recoverable source failure', () => {
  ...
  test('#6 shows an English error toast and recovers on retry when EN is active', async ({
    page,
    request,
  }) => {
    // Arrange — forecast upstream is down
    ...
    // Assert — recoverable error toast in active locale (US9/RF25)
    const toast = page.getByRole('alert')
    await expect(toast).toContainText('Weather data source unavailable.')
